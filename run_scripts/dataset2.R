...
library(googledrive)
name = "data_set"
  for (area_length_x in seq(100, 6000, by=1000)){
    for(initial_pop in c(1, 50, 100)) {
  for(death_r in seq(1, 10, 1)) {
    for(b in seq(0.15, 1, 0.1)){
      for(dd in seq(0.1, 1, 0.1)){
        for(d in seq(0.1, 1, 0.1)){
        for (auto_stop_at_plateau in c(TRUE)){
          for (sd_b in seq(0.5, 1, by=0.15)) {
            for (sd_d in seq(0.5, 1, by=0.15)){
